...
<script type="application/ld+json">
{
  ...
  "image": "{{asset('uploads')}}/{{ $categories[0]->image}}",
  "description": "UniqueProducts",
  "brand": {
    "@type": "Brand",
    "name": "UniqueProducts"
  },
  "sku": "{{$categories[0]->name.$categories[0]->id}}",
  "offers": {
    "@type": "Offer",
    "url": "{{route('categorywise',$categories[0]->id)}}",
                    ...
                    <ul id="categories">
                        ...
                        @foreach($categories as $category)
                            ...
                            <li>
                                ...
                                @if($page == 'categorywise')
                                    ...
                                    <a class="widget-link category_name" href="#{{$category->id}}" data-bs-toggle="collapse">{{$category->name}}</a>
                                     <!--<ul class="collapse show" id="{{$category->id}}" data-bs-parent="#categories">-->
                                    @else
                                    <a class="widget-link category_name collapsed" href="#{{$category->id}}" data-bs-toggle="collapse">{{$category->name}}</a>
                                ...
                                <!--<ul class="collapse {{$category->id}}" id="{{$category->id}}" data-bs-parent="#categories">-->
                                     <li><a class="widget-link" href="{{route('categorywise',$category->id)}}">View all</a></li>
                                     
                                   @foreach($category->subcategories as $subcategory)
                                    @php
                                      $subcategory_name = str_replace(' ','-',strtolower($subcategory->name))
                                    @endphp
                                     <li><a class="widget-link" href="{{route('subcategorywise',$subcategory->id)}}">{{ $subcategory->name }}</a></li>
                                    @endforeach
                                    @else
                        ...
                        @endforeach  
                          @else
                            ...
                            @endphp
                                     <li><a class="widget-link" href="{{route('single.product',$product->id)}}">{{ $product->name }}</a></li>
                    ...
                    <div class="mb-3">
                      ...
                      <datalist id="datalist-options">
                            ...
                            @foreach($products as $product)
                             ...
                             $product_name = str_replace(' ','-',strtolower($product->name))
                            @endphp
                            <a href="{{route('single.product',$product->id)}}"><option value="{{$product->name}}">{{$product->name}}</option></a>
                            @endforeach
                   ...
                   @foreach($category->subcategories as $subcategory)
                        <div class="col-md-4 col-sm-6 mb-grid-gutter">
                          @php
                            $subcategory_name = str_replace(' ','-',strtolower($subcategory->name))
                          @endphp
                      <div class="card card-product card-hover"><a class="card-img-top" href="{{route('subcategorywise', $subcategory->id)}}"><img src="{{asset('uploads')}}/{{ $subcategory->image}}" alt="Product thumbnail"></a>
                      <div class="card-body text-center">
                        <h4 class="fs-md fw-medium mt-2"><a class="meta-link" href="{{route('subcategorywise',$subcategory->id)}}">{{ $subcategory->name}}</a></h4>
                      </div>
                      </div>
                    </div>
                     @endforeach
                ...
                @else
                    ...
                    @if($product->getAllProductsBySubCategoryId($slug->id,$product->sub_category_id))
                        <div class="col-md-4 col-sm-6 mb-grid-gutter">
                            ...
                            $product_name = str_replace(' ','-',strtolower($product->name));
                            @endphp
                      <div class="card card-product card-hover"><a class="card-img-top" href="{{route('single.product',$product->id)}}"><img src="{{asset('uploads')}}/{{ $product->cover_photo}}" alt="Product thumbnail"></a>
                        <div class="card-body"><a class="meta-link fs-xs mb-1" href="#"></a>
                          <h3 class="fs-md fw-medium mb-2"><a class="meta-link" href="{{route('single.product',$product->id)}}">{{ $product->name}}</a></h3><span class="text-heading fw-semibold">${{ $product->price}}</span>
                        </div>
